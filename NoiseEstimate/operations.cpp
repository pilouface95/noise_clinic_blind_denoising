 ...
 * Website: http://mcolom.info ; Email: miguel@mcolom.info
 *
...
template <typename T>
int partition(T* input, int* pos, int p, int r)
{
    T pivot = input[r];
...
template <typename T>
void quick_select(T* input, int* pos, int p, int r, int k)
{
    if ( p == r ) return;
    int j = partition(input, pos, p, r);
    int length = j - p + 1;
    if ( length == k ) return;
    else if ( k < length ) quick_select(input, pos, p, j - 1, k);
    else quick_select(input, pos, j + 1, r, k - length);
}
#endif
