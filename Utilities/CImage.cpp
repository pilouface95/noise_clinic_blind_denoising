...
void CImage::create_channels(int num_channels, int Nx, int Ny) {
  this->channels = new float*[num_channels];

  for (int ch = 0; ch < num_channels; ch++)
    this->channels[ch] = new float[Nx*Ny];
...
CImage::CImage(int Nx, int Ny,
  ...
  this->bits = bits;
  //
  PRINT_VERBOSE("CImage %dx%d, %d bits, %d channels created\n",
    this->Nx, this->Ny, this->bits, num_channels);
...
CImage::~CImage() {
  PRINT_VERBOSE("CImage destroyed\n");
...
void CImage::load(char *filename) {
  ...
  PRINT_VERBOSE("Loaded image \"%s\", %dx%d, %d channels, %d bits\n",
...
void CImage::load_float_RGB(char *filename) {
  ...
  this->bits = 16;
  PRINT_VERBOSE("CImage loaded RGB (%dx%d), %d channels\n",
    this->Nx, this->Ny, num_channels);

  float *data_ALL = new float[num_channels*this->Nx*this->Ny];
...
void CImage::save_float_RGB(char *filename) {
  ...
  PRINT_VERBOSE("CImage saved RGB (%d, %d), %d channels\n",
    this->Nx, this->Ny, num_channels);
...
bool CImage::channels_are_equal() {
  for (int i = 0; i < this->get_width()*this->get_height(); i++) {
	float *ch_0 = this->get_channel(0);
    for (int c = 1; c < this->get_num_channels(); c++) {
		float *ch_c = this->get_channel(c);
		if (ch_0[i] != ch_c[i])
		  return false;
    }
  }
